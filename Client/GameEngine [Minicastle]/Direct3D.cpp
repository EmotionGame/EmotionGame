...
bool Direct3D::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : CreateDXGIFactory(__uuidof(IDXGIFactory), (void**)&factory)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : factory->EnumAdapters(0, &adapter)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : adapter->EnumOutputs(0, &adapterOutput", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numModes, NULL)", L"Error", MB_OK);
		return false;
	}
	...
	if (!displayModeList)
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : DXGI_MODE_DESC* displayModeList = new DXGI_MODE_DESC[numModes];", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : adapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numModes, displayModeList)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : adapter->GetDesc(&adapterDesc)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : wcstombs_s(&stringLength, m_videoCardDescription, 128, adapterDesc.Description, 128) != 0", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, &featureLevel, 1, D3D11_SDK_VERSION, &swapChainDesc, &m_swapChain, &m_device, NULL, &m_deviceContext)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBufferPtr)", L"Error", MB_OK);
		return false;
	}

	// 백 버퍼 포인터로 렌더 타겟 뷰를 생성합니다.
	if (FAILED(m_device->CreateRenderTargetView(backBufferPtr, NULL, &m_renderTargetView)))
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_device->CreateRenderTargetView(backBufferPtr, NULL, &m_renderTargetView)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_device->CreateTexture2D(&depthBufferDesc, NULL, &m_depthStencilBuffer)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_device->CreateDepthStencilState(&depthStencilDesc, &m_depthStencilState)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_device->CreateDepthStencilView(m_depthStencilBuffer, &depthStencilViewDesc, &m_depthStencilView)", L"Error", MB_OK);
		return false;
	}
	...
	{
		MessageBox(m_hwnd, L"Direc3D.cpp : m_device->CreateRasterizerState(&rasterDesc, &m_rasterState)", L"Error", MB_OK);
		return false;
	}
