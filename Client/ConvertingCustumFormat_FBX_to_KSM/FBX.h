...
private:
	...
	void ProcessGlobalOffPosition(FbxNode* pNode);
	void ProcessFinalTransform(FbxNode* pNode);
...
public: // 후에 private로 변경해야 합니다.
	...
	std::vector<bool>* m_HasAnimation = new std::vector<bool>;
	...
	std::vector<XMMATRIX>* m_GlobalOffPosition = new std::vector<XMMATRIX>;
	
	// 각 메시의 각 애님스택의 각 클러스터의 각 프레임에대한 최종변환행렬 저장
	std::unordered_map<unsigned int, AnimationVector>* m_FinalTransform = new std::unordered_map<unsigned int, AnimationVector>;
	/***** Model로 넘겨줄 데이터들 : 종료 *****/
};