...
void QuadTree::CreateTreeNode(QuadTreeNodeType* pNode, float positionX, float positionZ, float width)
{
// 노드의 위치와 크기를 저장한다.
...
CreateTreeNode(pNode->nodes[i], (positionX + offsetX), (positionZ + offsetZ), (width / 2.0f));
}
}
return;
}
